...
export const phaseInstructions: PhaseInstructionsType = {
  ...
  gender_practice: {
    ...
    content: `在這個階段，您需要對出現的詞語進行性別分類。
              請使用以下方式回答：
              - 鍵盤：按 E 鍵代表男性詞語，按 I 鍵代表女性詞語
              - 觸控：直接點擊左側區域（男性）或右側區域（女性）
  ...
  product_practice: {
    ...
    content: `在這個階段，您需要對出現的詞語進行產品類別分類。
            請使用以下方式回答：
            - 鍵盤：按 E 鍵代表電腦類產品，按 I 鍵代表護膚類產品
            - 觸控：直接點擊左側區域（電腦類）或右側區域（護膚類）
  ...
  combined_test_1: {
    ...
    content: `在這個階段，您需要同時對"性別詞語"和"產品詞語"進行分類。
            請使用以下方式回答：
            - 鍵盤：按 E 鍵代表「男性詞語」或「電腦類產品」，按 I 鍵代表「女性詞語」或「護膚類產品」
            - 觸控：點擊左側區域代表「男性」或「電腦類」，點擊右側區域代表「女性」或「護膚類」
  ...
  reversed_practice: {
    ...
    content: `注意！分類方向已經改變。
            請使用以下方式回答：
            - 鍵盤：按 E 鍵代表女性詞語，按 I 鍵代表男性詞語
            - 觸控：點擊左側區域（女性）或右側區域（男性）
  ...
  combined_test_2: {
    ...
    content: `在這個階段，您需要同時對"性別詞語"和"產品詞語"進行分類，但配對方式已改變。
            請使用以下方式回答：
            - 鍵盤：按 E 鍵代表「女性詞語」或「電腦類產品」，按 I 鍵代表「男性詞語」或「護膚類產品」
            - 觸控：點擊左側區域代表「女性」或「電腦類」，點擊右側區域代表「男性」或「護膚類」
